...
function entitlementSecret(): string {
  /* Dev default unchanged so existing local entitlement keys keep validating after rename. */
  return (
...
/**
 * Validates `adent1.<base64urlPayload>.<base64urlHmac>` where HMAC-SHA256(secret, payload) matches.
...
export function validateProEnterpriseKey(raw: string): boolean {
  ...
  if (parts.length !== 3 || parts[0] !== KEY_PREFIX) return false;
  const [, payloadB64, sigB64] = parts;
  if (!payloadB64 || !sigB64) return false;
  const expected = createHmac('sha256', entitlementSecret()).update(payloadB64).digest('base64url');
  try {
