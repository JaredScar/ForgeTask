...
### 6.1 — Auto-Refresh (MISSING)

**Current state:** The logs page is static after load — new logs generated by the engine are not visible without a page reload.

**What to build:**
- Add an IPC event `logs:new` that the engine emits via `mainWindow.webContents.send()` after each workflow execution completes.
- In `ipc.service.ts`, expose a `onNewLog()` observable (using `ipcRenderer.on`).
- In `logs-page.component.ts`, subscribe to this observable and prepend the new log entry to the top of the list.
- Show a "New logs available" toast/banner when the user is scrolled down, with a "Scroll to top" action.

**Files:** `electron/engine/automation-engine.ts`, `electron/ipc-handlers.ts`, `src/app/core/services/ipc.service.ts`, `src/app/features/logs/logs-page.component.ts`

---

### 6.2 — Real-Time Step Progress

**Current state:** Log step detail is only available after a run completes.

**What to build:**
- While a workflow is running, emit `logs:stepProgress` events from the engine with `{ logId, stepIndex, status, message }`.
- Open a "Live Run" panel that shows each step updating in real time as they execute.
